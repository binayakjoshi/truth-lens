...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # Adjust this to your explicit NestJS service URL in production
...
try:
    ...
    print(f"⚠️ Model weight file missing or incompatible: {e}. Running with uninitialized weights.")
...
async def predict_image(file: UploadFile = File(...)):
    ...
    if file.content_type not in ["image/jpeg", "image/png", "image/jpg"]:
        raise HTTPException(status_code=400, detail="Invalid media asset format. Must be JPEG or PNG.")
    ...
    try:
        ...
        predicted_class_idx, probabilities, heatmap = process_inference_with_gradcam(input_tensor)
        ...
        raise HTTPException(status_code=500, detail=f"Inference execution engine processing fault: {str(e)}")
